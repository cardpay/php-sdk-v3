...
class MobileApi
{
     ...
     * @throws \Cardpay\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return \Cardpay\model\MobileVerificationResponse
     */
    public function verify($authorization, $request)
    {
        list($response) = $this->verifyWithHttpInfo($authorization, $request);
        return $response;
    }
     ...
     * @throws \Cardpay\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \Cardpay\model\MobileVerificationResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function verifyWithHttpInfo($authorization, $request)
    {
        $returnType = '\Cardpay\model\MobileVerificationResponse';
        $request = $this->verifyRequest($authorization, $request);
        ...
        try {
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\Cardpay\model\MobileVerificationResponse',
                        $e->getResponseHeaders()
                    );
                    $e->setResponseObject($data);
                    break;
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function verifyAsync($authorization, $request)
    {
        return $this->verifyAsyncWithHttpInfo($authorization, $request)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function verifyAsyncWithHttpInfo($authorization, $request)
    {
        $returnType = '\Cardpay\model\MobileVerificationResponse';
        $request = $this->verifyRequest($authorization, $request);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function verifyRequest($authorization, $request)
    {
        // verify the required parameter 'authorization' is set
        if ($authorization === null || (is_array($authorization) && count($authorization) === 0)) {
            throw new \InvalidArgumentException(
        ...
        $resourcePath = '/api/mobile/verify';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
